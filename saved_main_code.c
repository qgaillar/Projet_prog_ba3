...
double Fg_totale_x(x_Asteroid_crash_test, y_Asteroid_crash_test, i){
    ...
    double cos_Mercure = (fabs(x_Asteroid_crash_test - Mercure.coord_x[i]) / np.sqrt((x_Asteroid_crash_test - Mercure.coord_x[i])**2 + (y_Asteroid_crash_test - Mercure.coord_y[i])**2));
